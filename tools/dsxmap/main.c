...
int MiniMapBSPMain(int argc, char **argv)
{
	...
	{
		Sys_Printf
			("Usage: dsxmap [-v] -minimap [-size n] [-sharpen f] [-samples n | -random n] [-o filename.png] [-minmax Xmin Ymin Zmin Xmax Ymax Zmax] <mapname>\n");
		return 0;
	}
...
int FixAAS(int argc, char **argv)
{
	...
	if(argc < 2)
	{
		Sys_Printf("Usage: dsxmap -fixaas [-v] <mapname>\n");
		return 0;
	}
...
int AnalyzeBSP(int argc, char **argv)
{
	...
	if(argc < 1)
	{
		Sys_Printf("Usage: dsxmap -analyze [-lumpswap] [-v] <mapname>\n");
		return 0;
	}
...
int ScaleBSPMain(int argc, char **argv)
{
	...
	if(argc < 3)
	{
		Sys_Printf("Usage: dsxmap [-v] -scale [-tex] <value> <mapname>\n");
		return 0;
	}
...
int ConvertBSPMain(int argc, char **argv)
{
	...
	if(argc < 1)
	{
		Sys_Printf("Usage: dsxmap -scale <value> [-v] <mapname>\n");
		return 0;
	}
...
int ConvertMapMain(int argc, char **argv)
{
	...
	if(i != argc - 1)
	{
		Error("usage: dsxmap [general options] -map2map [-<switch> [-<switch> ...]] <mapname.map>\n"
			  "\n"
			  "Switches:\n"
			  " -v                     = verbose output\n"
...
int main(int argc, char **argv)
{
	...
	Sys_Printf("dsxmap (Tr3B)  - v" Q3MAP_VERSION "\n");
