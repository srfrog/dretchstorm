...

	{
		...
		if(glConfig.driverType != GLDRV_OPENGL3 && optimize)
		{
			...
			if(glslopt_get_status(shaderOptimized))
			{
				...
				qglShaderSourceARB(shader, 1, (const GLcharARB **)&newSource, &length);
			}
			else
			{
...
//      qglBindAttribLocationARB(program, ATTR_INDEX_TEXCOORD2, "attr_TexCoord2");

//  if(attribs & ATTR_TEXCOORD3)
//      qglBindAttribLocationARB(program, ATTR_INDEX_TEXCOORD3, "attr_TexCoord3");
...
//	shaderProgram_t *shaderProgram;
	...
	tr.vertexLightingShader_DBS_world.u_DeformBulge =
		qglGetUniformLocationARB(tr.vertexLightingShader_DBS_world.program, "u_DeformBulge");
...
//		qglGetUniformLocationARB(tr.forwardLightingShader_DBS_directional.program, "u_InverseVertexColor");
	...
	tr.cameraEffectsShader.u_ColorTextureMatrix = qglGetUniformLocationARB(tr.cameraEffectsShader.program, "u_ColorTextureMatrix");
...
void Tess_DrawElements()
{
	...
	if(glState.currentVBO && glState.currentIBO)
	{
		...
		{
			qglMultiDrawElements(GL_TRIANGLES, tess.multiDrawCounts, GL_INDEX_TYPE, (const GLvoid**) tess.multiDrawIndexes, tess.multiDrawPrimitives);
