...
public class PlayerState extends EntityState implements Cloneable, PlayerStateAccess
{
	...
	private int					events[];				// MAX_PS_EVENTS];
	private int					eventParms[];			// MAX_PS_EVENTS];
	...
	public PlayerState(int commandTime, int pmType, int pmFlags, int pmTime, int bobCycle, Vector3f origin, Vector3f velocity, int weaponTime, int gravity,
			int speed, int deltaPitch, int deltaYaw, int deltaRoll, int groundEntityNum, int legsTimer, int legsAnim, int torsoTimer, int torsoAnim,
			int movementDir, Vector3f grapplePoint, int eFlags, int eventSequence, int[] events, int[] eventParms, int externalEvent, int externalEventParm,
			int externalEventTime, int clientNum, int weapon, int weaponState, Angle3f viewAngles, int viewHeight, int damageEvent, int damageYaw,
			int damagePitch, int damageCount, int[] stats, int[] persistant, int[] powerups, int[] ammo, int generic1, int loopSound, int jumppadEnt, int ping,
			int pmoveFramecount, int jumppadFrame, int entityEventSequence)
	{
		super(clientNum);
	...
	public String toString()
	{
		return String
				.format(
						...
						Arrays.toString(ammo), bobCycle, clientNum, commandTime, damageCount, damageEvent, damagePitch, damageYaw, deltaPitch, deltaRoll,
						deltaYaw, eFlags, entityEventSequence, Arrays.toString(eventParms), eventSequence, Arrays.toString(events), externalEvent,
						externalEventParm, externalEventTime, generic1, grapplePoint, gravity, groundEntityNum, jumppad_ent, jumppad_frame, legsAnim,
						legsTimer, loopSound, movementDir, origin, Arrays.toString(persistant), ping, pm_flags, pm_time, pm_type, pmove_framecount, Arrays
								.toString(powerups), speed, Arrays.toString(stats), torsoAnim, torsoTimer, velocity, viewAngles, viewHeight, weapon,
						weaponState, weaponTime);
	}

	public Object clone()
	{

		PlayerState ps = new PlayerState(commandTime, pm_type.ordinal(), pm_flags, pm_time, bobCycle, new Vector3f(origin), new Vector3f(velocity), weaponTime,
				gravity, speed, deltaPitch, deltaYaw, deltaRoll, groundEntityNum, legsTimer, legsAnim, torsoTimer, torsoAnim, movementDir, new Vector3f(
						grapplePoint), eFlags, eventSequence, events.clone(), eventParms.clone(), externalEvent, externalEventParm, externalEventTime,
				clientNum, weapon, weaponState, new Angle3f(viewAngles), viewHeight, damageEvent, damageYaw, damagePitch, damageCount, stats.clone(),
				persistant.clone(), powerups.clone(), ammo.clone(), generic1, loopSound, jumppad_ent, ping, pmove_framecount, jumppad_frame,
				entityEventSequence);
